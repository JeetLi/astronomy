...
import { apodCustomProps, apodProps } from "../interfaces/interfaces";
const DateHelper = {
  ...
  validDayFilter: (arrUrls: Array<apodCustomProps>) => {
  ...
  daysCombine: async (startDate: string, numberOfDays: number) => {
    const key = "R7OOLMfPJuY2QfRtByBmCxoLsxo22jLOWiBiJSB5";
    let nowDate = startDate;
    let url = `/apod?api_key=${key}&date=${nowDate}`;
    const arrDayUrls: Array<apodCustomProps> = [];
